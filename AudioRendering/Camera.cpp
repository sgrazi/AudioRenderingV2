...
void Camera::moveCam() {
	if (movecam) movecam = false;
	else {
		movecam = true;
		SDL_WarpMouseInWindow(win, unsigned int(width / 2), unsigned int(height / 2));	//Debe ser que la posicion del mouse cambia entre que se ejecuta esta funcion y la de update
		int x, y;
		SDL_GetMouseState(&x, &y);
		mouseX = x;
		mouseY = y;
