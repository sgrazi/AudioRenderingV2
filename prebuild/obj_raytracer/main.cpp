...
void screen(AudioFile<float> *audio)
{
	...
	float base = Context::get_base_power();
	float ray_distance = Context::get_ray_distance_threshold();
	float ray_energy = Context::get_ray_energy_threshold();


	std::cout << "base" << base << std::endl;
	std::cout << "ray_distance" << ray_distance << std::endl;
	std::cout << "ray_energy" << ray_energy << std::endl;

	renderer->setBasePower(base);
	renderer->setThresholds(ray_distance, ray_energy, Context::get_ray_max_bounces());
